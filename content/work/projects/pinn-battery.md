...
**PyGNN** – or Graph Neural Networks for batteries – model the battery as a graph where electrodes, electrolyte, and particles are nodes connected by physical relationships. This is excellent for capturing spatial heterogeneity but less suited for temporal degradation prediction. 
...
The field is still young. Master's theses from 2024 note that while PINNs show enormous promise for battery degradation modeling, achieving high accuracy for long-term predictions remains challenging.  The nonlinear dynamics of late-stage aging – where lithium plating causes sudden capacity dives – are particularly difficult to capture.
